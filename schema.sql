...
drop policy if exists "tabs: users own their rows"     on public.tabs;
drop policy if exists "tasks: users own their rows"    on public.tasks;
drop policy if exists "settings: users own their rows" on public.user_settings;
